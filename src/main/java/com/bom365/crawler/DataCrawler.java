...
public class DataCrawler extends CrawlingUtil{
	private final static String USER_AGENT = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.152 Safari/537.36";
	
	private Connection connection;
	
	public void takeSiteConnection(String URL) {
		connection = Jsoup.connect(URL)
					.userAgent(USER_AGENT)
					.method(Connection.Method.GET)
					.ignoreContentType(true);
	}
	public Document takeDocumentConnection() throws IOException {
		return connection.get();
		
	}
	public List<AnimalDto> start(int page) {
		//DataCrawler crawler = new DataCrawler();
		List<AnimalDto> animalDtoList = null;
		
		String URL = getURL(page);
		
		takeSiteConnection(URL);
		
		try {
			Document document = takeDocumentConnection();
	...
	public List<AnimalDto> AnimalArrayToDtoList(ArrayList<ArrayList<String>> ageWeightEtcList,ArrayList<String> animalNameList, ArrayList<String> imgElements) {
		...
		for(int i = 0; i < size; i++) {
			...
			animalDto.setAnimalImage("https://ekara.org"+imgElements.get(i));
			...
			animalDto.setNeutering(ageWeightEtc.get(4));
