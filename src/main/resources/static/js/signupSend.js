...
//<input type="hidden" th:field="*{memberFormDto.duplicate}" value="">
var status = {
    isIdRock : false,
...
function send() {
    var supporterId = document.querySelector('input[name=supporterId]');
    var name  = document.querySelector('input[name=supporterName]');
    var supporterPassword = document.querySelector('input[name=supporterPassword]');
    ...
    var detailedAddress = document.querySelector('input[name=detailedAddress]');
    ...
    $.ajax({
        ...
        success : function(result,status,xhr) {
            if(result.status == 200) {
                ...
                status.message.duplicate = result.duplicate;
            }
